 ...
 *
 * The bound exists for the same reason `MAX_REPLAY_FAN_OUT` does, and for one
 * more. Every requeued row becomes a fan-out, and every fan-out becomes real
 ...
 *
 * 100 rather than 50 (`MAX_REPLAY_FAN_OUT`): this is a recovery path, not a
 ...
 * happens AFTER the transaction commits - a hundred fan-outs entering the
 ...
 * would either duplicate work the router is doing right now or re-run a fan-out
