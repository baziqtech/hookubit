...
| **Delivery** | One event to one endpoint, with its own retry chain. Created at fan-out, before anything is sent, so the ledger is the record of what should arrive. |
...
| **Event** | A fact your system published, once, with its payload. Its status is the ingest and fan-out state, not a delivery outcome. |
...
| **Fan-out** | Turning one accepted event into one delivery row per matching subscription. Pinned to the subscriptions that existed when the event was accepted. |
| **Fan-out cursor** | On an outbox row, the point a wide fan-out reached before it stopped. A requeue resumes from it. |
...
| **Requeue** | Putting a parked outbox row back so the router runs the fan-out it never ran. Not a replay. |
