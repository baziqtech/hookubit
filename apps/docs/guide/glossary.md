...
| **Auto-disable** | The platform switching off an endpoint whose circuit breaker has been open continuously for 72 hours (by default). Sets `status: disabled` and a `disabled_reason` beginning `auto-disabled:`; fan-out stops creating deliveries for it. Re-enabling arms one probe. [05](./05-retries-and-delivery.md#auto-disable) |
...
| **Delivery** | One (event, endpoint) pair with its own state, retry budget and attempt history. Created by fan-out; identified by `del_…` and sent as `Webhook-Delivery-Id`. [01](./01-concepts.md) |
...
| **Fan-out** | Turning one accepted event into one delivery row per matching enabled subscription, as they existed when the event was accepted. Exactly-once per (event, endpoint). [01](./01-concepts.md#the-path-of-an-event) |
...
| **Park / requeue** | An event the router gave up fanning out is *parked* (`status: failed`, listed on *Outbox*); *requeue* returns it to the queue so fan-out runs. Not a replay: there were no deliveries to replay. [08](./08-troubleshooting.md#a-parked-event) |
...
| **Retry policy** | The schedule frozen onto a delivery at fan-out: strategy, attempts, delays, jitter, wall-clock budget. Per endpoint, with a project default and a built-in default. [05](./05-retries-and-delivery.md#the-default-schedule) |
