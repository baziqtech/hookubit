...
**Replay-to-all uses the original fan-out, not today's subscriptions.** "Every endpoint the event originally reached" is read off the existing delivery rows. An endpoint subscribed *since* the event was published does not receive it, and an endpoint whose subscription has since been narrowed or deleted still does. The operator asking for a replay is asking for what happened, not what would happen if the event were published now.
...
**Replay never re-runs subscription matching.** If the event never reached an endpoint because the subscription did not match, the endpoint was paused, or the event is still parked before fan-out, replay has nothing to work from. The last case has its own tool: [requeue](./08-troubleshooting.md#a-parked-event).
...
| Replay-to-all would create more than **50** deliveries | `409` `limit_exceeded`, with `{limit: 50, current: N, resource: "replay_fan_out"}`. Replay per endpoint instead. |
...
| Any endpoint in a replay-to-all is refused | The **whole** replay is refused and nothing is created - so the error names the endpoint that caused it rather than leaving a half-replayed fan-out. |
